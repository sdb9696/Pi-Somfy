...
class Event:
    ...
    def __init__(
        ...
        if (repeat_value == "weekday") and not all(
            elem in ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
            for elem in repeat_value
        ):
            raise ValueError(
                "%s is not a valid value for REPEATVALUE (weekday)." % repeat_value
            )
        if (repeat_value == "once") and not (
            datetime.datetime.strptime(repeat_value, "%Y/%m/%d")
        ):
            raise ValueError(
                "%s is not a valid value for REPEATVALUE (once)." % repeat_value
            )
        ...
        if (time_type == "clock") and not time.strptime(time_value, "%H:%M"):
            raise ValueError(
                "%s is not a valid value for TIMEVALUE (clock)." % time_value
            )
        astro_parts = re.split(r"\+|\-", time_value)
        if (time_type == "astro") and not (
            (astro_parts[0] in ("sunset", "sunrise"))
            and (
                (len(astro_parts) == 1)
                or (astro_parts[1] == None or int(astro_parts[1]))
            )
        ):
            raise ValueError(
                "%s is not a valid value for TIMEVALUE (astro)." % time_value
            )
        self.time_value = time_value

        # if not ((isinstance(shutterAction, str)) and ((shutterAction.startswith("up") or shutterAction.startswith("down")))):
        if not (
            shutter_action.startswith("up")
            or shutter_action.startswith("down")
            or shutter_action.startswith("stop")
        ):
...
class Schedule:
    ...
    def add_repeat_event_by_time(
        self, shutter_ids, shutter_action, hour, minute, weekdays
    ):
        try:
            evt = Event(
                "active",
    ...
    def edit_schedule(
        ...
        if (id not in self.schedule) or (id not in self.config.schedule):
    ...
    def get_schedule_as_dict(self):
        ...
        for id, evt in self.schedule.items():
            if evt.active != "deleted":
                item = {
                    "active": evt.active,
                    "repeatType": evt.repeat_type,
                    "repeatValue": evt.repeat_value,
                    "timeType": evt.time_type,
                    "timeValue": evt.time_value,
                    "shutterIds": evt.shutter_ids,
                    "shutterAction": evt.shutter_action,
                }
                obj[id] = item
        return obj
...
class Scheduler(threading.Thread):
    ...
    def update_schedule(self):
        ...
        for id, event in self.schedule.get_schedule().items():
            if (event.active == "active") and (
                ((event.repeat_type == "weekday") and (weekday in event.repeat_value))
                or ((event.repeat_type == "once") and (date == event.repeat_value))
            ):
                if event.time_type == "clock":
                    event_time = datetime.time(
                        int(event.time_value.split(":")[0]),
                        int(event.time_value.split(":")[1]),
                        0,
                    )
                elif (event.time_type == "astro") and (
                    event.time_value.startswith("sunrise")
                ):
                    event_time = (
                        sunrise
                        + datetime.timedelta(minutes=int(event.time_value[7:] or 0))
                    ).time()
                elif (event.time_type == "astro") and (
                    event.time_value.startswith("sunset")
                ):
                    event_time = (
                        sunset
                        + datetime.timedelta(minutes=int(event.time_value[6:] or 0))
                    ).time()

                if event_time > datetime.datetime.now().time():
                    event_time_str = "%02d:%02d" % (event_time.hour, event_time.minute)
                    if event_time_str not in self.current_schedule:
                        self.current_schedule[event_time_str] = []
                    self.current_schedule[event_time_str].append(
                        [event.shutter_ids, event.shutter_action]
                    )
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set():
            ...
            if (self.last_schedule_update_time < current_schedule_update_time) or (
                self.weekday != datetime.datetime.today().weekday()
            ):
                        ...
                        for shutter_id in event_detail[0]:
                            try:
                                ...
                                if event_detail[1].startswith("up"):
                                    ...
                                    if 0 < s1 < 100:
                                        if (
                                            self.shutter.get_position(shutter_id) < s1
                                        ):  # Is Shutter below requested Position?
                                    ...
                                    if 0 < s1 < 100:
                                        if (
                                            self.shutter.get_position(shutter_id) > s1
                                        ):  # Is Shutter above requested Position?
