...
class Shutter:
    ...
    def get_shutter_state(self, shutter_id, initial_position = None):
        with self.shutter_state_lock:
            if shutter_id not in self.shutter_state_list:
                self.shutter_state_list[shutter_id] = self.ShutterState(initial_position)
    ...
    def wait_and_set_final_position(self, shutter_id, time_to_wait, new_position):
        ...
        LOGGER.debug("["+self.config.shutters[shutter_id]['name']+"] Waiting for operation to complete for " + str(time_to_wait) + " seconds")
        ...
        if state.last_command_time == old_last_command_time:
            LOGGER.debug("["+self.config.shutters[shutter_id]['name']+"] Set new final position: " + str(new_position))
            self.set_position(shutter_id, new_position)
        else:
            LOGGER.debug("["+self.config.shutters[shutter_id]['name']+"] Discard final position. Position is now: " + str(state.position))
    ...
    def lower(self, shutter_id):
        ...
        time_to_wait = state.position/100*self.config.shutters[shutter_id]['durationDown']
        t = threading.Thread(target = self.wait_and_set_final_position, args = (shutter_id, time_to_wait, 0))
        t.start()
    ...
    def lower_partial(self, shutter_id, percentage):
        ...
        self.send_command(shutter_id, self.BUTTON_DOWN, self.config.SendRepeat)
        state.register_command('down')
        time.sleep((state.position-percentage)/100*self.config.shutters[shutter_id]['durationDown'])
        LOGGER.info("["+self.config.shutters[shutter_id]['name']+"] Stop at partial position requested")
    ...
    def rise(self, shutter_id):
        ...
        # wait and set final position only if not interrupted in between
        time_to_wait = (100-state.position)/100*self.config.shutters[shutter_id]['durationUp']
        t = threading.Thread(target = self.wait_and_set_final_position, args = (shutter_id, time_to_wait, 100))
        t.start()
    ...
    def rise_partial(self, shutter_id, percentage):
        ...
        self.send_command(shutter_id, self.BUTTON_UP, self.config.SendRepeat)
        state.register_command('up')
        time.sleep((percentage-state.position)/100*self.config.shutters[shutter_id]['durationUp'])
        LOGGER.info("["+self.config.shutters[shutter_id]['name']+"] Stop at partial position requested")
        self.send_command(shutter_id, self.BUTTON_STOP, self.config.SendRepeat)
    ...
    def stop(self, shutter_id):
        ...
        seconds_since_last_command = int(round(time.monotonic() - state.last_command_time))
        LOGGER.debug("["+shutter_id+"] Seconds since last command: " + str(seconds_since_last_command))
        ...
        if state.last_command_direction == 'up':
            if seconds_since_last_command > 0 and seconds_since_last_command < setup_duration_up:
                duration_percentage = int(round(seconds_since_last_command/setup_duration_up * 100))
                LOGGER.debug("["+shutter_id+"] Up duration percentage: " + str(duration_percentage) + ", State position: "+ str(state.position))
            ...
            if seconds_since_last_command > 0 and seconds_since_last_command < setup_duration_down:
                duration_percentage = int(round(seconds_since_last_command/setup_duration_down * 100))
                LOGGER.debug("["+shutter_id+"] Down duration percentage: " + str(duration_percentage) + ", State position: "+ str(state.position))
        ...
        if fallback == True: # Let's assume it will end on the intermediate position ! If it exists !
            intermediate_position = self.config.shutters[shutter_id]['intermediatePosition']
            if (intermediate_position == None) or (intermediate_position == state.position):
                LOGGER.info("["+shutter_id+"] Stay stationary.")
                new_position = state.position
            else:
                LOGGER.info("["+shutter_id+"] Motor expected to move to intermediate position "+str(intermediate_position))
                if state.position > intermediate_position:
                    state.register_command('down')
                    time_to_wait = abs(state.position - intermediate_position) / 100*self.config.shutters[shutter_id]['durationDown']
                else:
                    state.register_command('up')
                    time_to_wait = abs(state.position - intermediate_position) / 100*self.config.shutters[shutter_id]['durationUp']
                # wait and set final intermediate position only if not interrupted in between
                t = threading.Thread(target = self.wait_and_set_final_position, args = (shutter_id, time_to_wait, intermediate_position))
                t.start()
                return
    ...
    # prog button (at least thirteen times after the original frame to activate the registration.
        ...
        try:
            ...
            LOGGER.info(f"Remote  :       0x{teleco:02X} ({self.config.shutters[shutter_id]['name']})")
            ...
            if not (self.config.Rfm69Enabled):
                ...
                with SomfyRfm69Tx(self.config.Rfm69ResetGPIO, self.TXGPIO, spichannel=self.config.Rfm69SPIChannel, pigpiohost=self.config.PIGPIOHost, pigpioport=self.config.PIGPIOPort, pigpio_connect_timeout=self.config.PIGPIO_Connect_Timeout) as somfy_rfm69_tx:
...
class OperateShutters:
    ...
    def process_command(self, args):
        ...
        if args.shutter_name:
            ...
            if not shutter_id:
                raise click.ClickException(f"Shutter '{args.shutter_name}' not found in config.")
    ...
    def _schedule_dusk_dawn(self, args, shutter_id, dusk_dawn_offsets):
        ...
        self.schedule.add_repeat_event_by_sunrise([shutter_id], 'up', dusk_dawn_offsets[1], weekdays)
        self.schedule.add_repeat_event_by_sunset([shutter_id], 'down', dusk_dawn_offsets[0], weekdays)
